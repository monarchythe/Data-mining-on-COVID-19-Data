...
# Example: Using only the top 8 variables with non-zero importance
...
# Adjusting the tree complexity parameter to reduce depth and simplify
simple_tree_clean <- rpart(
  bad ~ ., 
  data = cases_train_subset, 
  method = "class",  # Classification tree
  control = rpart.control(cp = 0.02, minsplit = 20, maxdepth = 4)  # Adjusting for simplicity
)
...
# Extract CP values and corresponding cross-validation errors (xerror)
cp_values <- cp_table[, "CP"]
xerror_values <- cp_table[, "xerror"]

# Plot the Complexity Parameter (CP) vs Cross-Validation Error
plot(cp_values, xerror_values, 
     type = "b", 
     col = "blue", 
     pch = 19, 
     main = "Complexity Parameter vs. Accuracy", 
     xlab = "Complexity Parameter (CP)", 
     ylab = "Cross-Validation Error (xerror)",
     lwd = 2)

# Add a vertical line at the optimal CP (the one that minimizes xerror)
